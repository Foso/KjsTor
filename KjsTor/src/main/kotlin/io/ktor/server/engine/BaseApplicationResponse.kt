 ...
 */
abstract class BaseApplicationResponse(override val call: ApplicationCall) :
        ...
        when {
            ...
            !transferEncodingSet -> {
                when (content) {
                    ...
                    is OutgoingContent.NoContent -> headers.append(
                        HttpHeaders.ContentLength, "0", safeOnly = false)
...
//            }
    ...
    companion object {
        ...
        val EngineResponseAtributeKey =
            AttributeKey<BaseApplicationResponse>(
                "EngineResponse"
            )
        ...
        fun setupSendPipeline(sendPipeline: ApplicationSendPipeline) {
            sendPipeline.intercept(ApplicationSendPipeline.Engine) { response ->
                console.log("vsetupSendPipeline")
                if (response !is OutgoingContent) {
                    throw IllegalArgumentException("Response pipeline couldn't transform '${response}' to the OutgoingContent")
                }
